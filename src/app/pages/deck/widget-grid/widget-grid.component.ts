...
@Component({
  ...
  standalone: true,
  imports: [NgStyle, WidgetContentComponent],
})
export class WidgetGridComponent implements AfterContentInit, OnDestroy {
  ...
  calculateWidgetSize(): void {
    ...
    this.wrapperPaddingY =
      parseInt(wrapperStyle.getPropertyValue('padding-top')) +
      parseInt(wrapperStyle.getPropertyValue('padding-bottom'));
    this.wrapperWidth =
      (this.wrapperElement?.nativeElement.offsetWidth ?? 0) -
      this.wrapperPaddingX;
    this.wrapperHeight =
      (this.wrapperElement?.nativeElement.offsetHeight ?? 0) -
      this.wrapperPaddingY;
    ...
    this.widgetSpacingPoints =
      ((this.macroDeckService.buttonSpacing / 100) * this.buttonSize * 72) /
      96 /
      2;
    WidgetGridComponent.borderRadiusPoints =
      ((this.macroDeckService.buttonRadius / 100) * this.buttonSize * 72) /
      96 /
      2;
  ...
  getWidgetStyle(index: number) {
    ...
    const xOffset =
      this.wrapperWidth / 2 -
      (this.macroDeckService.columns * this.buttonSize) / 2; // Offset to center items horizontally
    const yOffset =
      this.wrapperHeight / 2 -
      (this.macroDeckService.rows * this.buttonSize) / 2; // Offset to center items vertically
