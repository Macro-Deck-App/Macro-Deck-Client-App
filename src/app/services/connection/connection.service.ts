...
export class ConnectionService {
  ...
  public async getConnections(): Promise<Connection[]> {
    ...
    const connections = JSON.parse(connectionsData).sort(
      (a: Connection, b: Connection) => (a.index ?? 0) - (b.index ?? 0),
    );
  ...
  public async addUpdateConnection(connection: Connection) {
    ...
    if (!connection.id) {
      ...
      const existingIndex = connectionsObject.findIndex(
        (x: Connection) => x.id === connection.id,
      );
      if (existingIndex > -1) {
  ...
  public async deleteConnection(id: string) {
    ...
    await this.storage.set(
      this.connectionsStorageKey,
      JSON.stringify(connectionsObject),
    );
  }
}
